...
os.environ["PW_TEST_SCREENSHOT_NO_FONTS_READY"] = (
    "1"  # https://github.com/microsoft/playwright/issues/35972
)
...
BrowserContextInstance = (
    InstanceOf[PatchrightBrowserContext] | InstanceOf[PlaywrightBrowserContext]
)
...
ElementHandleInstance = (
    InstanceOf[PatchrightElementHandle] | InstanceOf[PlaywrightElementHandle]
)
FrameLocator = PatchrightFrameLocator | PlaywrightFrameLocator
FrameLocatorInstance = (
    InstanceOf[PatchrightFrameLocator] | InstanceOf[PlaywrightFrameLocator]
)
...
GLOBAL_PLAYWRIGHT_API_OBJECT = (
    None  # never instantiate the playwright API object more than once per thread
)
GLOBAL_PATCHRIGHT_API_OBJECT = (
    None  # never instantiate the patchright API object more than once per thread
)
GLOBAL_PLAYWRIGHT_EVENT_LOOP = (
    None  # track which event loop the global objects belong to
)
GLOBAL_PATCHRIGHT_EVENT_LOOP = (
    None  # track which event loop the global objects belong to
)
...
def require_initialization(func):
    ...
    assert asyncio.iscoroutinefunction(
        func
    ), "@require_initialization only supports decorating async methods on BrowserSession"
    ...
    async def wrapper(self, *args, **kwargs):
        try:
            ...
            if "TargetClosedError" in str(
                type(e)
            ) or "context or browser has been closed" in str(e):
                self.logger.warning(
...
class BrowserSession(BaseModel):
    ...
    agent_current_page: PageInstance | None = (
    ...
    human_current_page: PageInstance | None = (
    ...
    async def start(self) -> Self:
        ...
        async with asyncio.timeout(
            60
        ):  # 60 overall second timeout for entire launching process
            async with self._start_lock:
                if self.initialized:
                    if self.is_connected():
                        ...
                        self.logger.warning(
                            f"💔 Browser {self._connection_str} has gone away, {next_step}..."
                        )
                ...
                try:
                    ...
                    ), f"Failed to connect to or create a new BrowserContext for browser={self.browser}"
    ...
    def _connection_str(self) -> str:
        ...
        binary_name = (
            Path(self.browser_profile.executable_path)
            .name.lower()
            .replace(" ", "-")
            .replace(".exe", "")
        ...
        return (
            ...
            if self.cdp_url
                ...
                f"wss_url={self.wss_url}"
                if self.wss_url
                else (
                    f"browser_pid={self.browser_pid}"
                    if self.browser_pid
            ...
            async with self._start_lock:
                ...
                if self.is_connected():
                    try:
                        ...
                        self.logger.warning(
                            f"⚠️ Failed to save auth storage state before stopping: {type(e).__name__}: {e}"
                        )
                ...
                if self.browser_context or self.browser:
                    self.logger.info(
                        f"🛑 Closing {self._connection_str} browser context {self.browser_context or self.browser}"
                    )
                    try:
                        ...
                        try:
                            async with asyncio.timeout(
                                5
                            ):  # 5 second timeout for close operation
                                await (
                                    self.browser
                                    or self.browser_context.browser
                                    or self.browser_context
                                ).close()
                        except TimeoutError:
                            self.logger.warning(
                                "⏱️ Timeout while closing browser/context, has it become unresponsive?"
                            )
                    except Exception as e:
                        self.logger.warning(
                ...
                if self.browser_pid:
                    try:
                        ...
                        self.logger.info(
                            f" ↳ Killing browser_pid={self.browser_pid} {_log_pretty_path(executable_path)}"
                        )
    ...
    async def kill(self) -> None:
        ...
        if self.playwright:
            try:
                ...
                self.logger.warning(
                    f"❌ Error stopping playwright: {type(e).__name__}: {e}"
                )
            finally:
    ...
    def _kill_child_processes(self) -> None:
        ...
        if not self.browser_profile.keep_alive and self.browser_pid:
            try:
                ...
                self.logger.warning(
                    f"Error force-killing browser in BrowserSession.__del__: {type(e).__name__}: {e}"
                )
    ...
    async def setup_playwright(self) -> None:
        ...
        if self.browser_profile.stealth:
            ...
            self.browser_profile.channel = (
                self.browser_profile.channel or BrowserChannel.CHROME
            )
            self.logger.info(
            ...
            if not self.playwright:
                ...
                if (
                    GLOBAL_PATCHRIGHT_API_OBJECT
                    and GLOBAL_PATCHRIGHT_EVENT_LOOP != current_loop
                ):
                    self.logger.debug(
                ...
                if GLOBAL_PATCHRIGHT_API_OBJECT and not should_recreate:
                    try:
                        ...
                        self.logger.debug(
                            f"Detected invalid patchright instance: {type(e).__name__}. Creating new instance."
                        )
                        should_recreate = True
                ...
                if GLOBAL_PATCHRIGHT_API_OBJECT:
                    ...
                    GLOBAL_PATCHRIGHT_API_OBJECT = (
                        await async_patchright().start()
                    )  # never start more than one per thread, never try to close it if any other code might be trying to use it, cannot be re-opened once closed
            ...
            if (
                self.browser_profile.channel
                and self.browser_profile.channel != BrowserChannel.CHROME
            ):
                self.logger.info(
            ...
            self.browser_profile.channel = (
                self.browser_profile.channel or BrowserChannel.CHROMIUM
            )
            if not self.playwright:
                # Check if the global object is still valid
                should_recreate = False
                ...
                if (
                    GLOBAL_PLAYWRIGHT_API_OBJECT
                    and GLOBAL_PLAYWRIGHT_EVENT_LOOP != current_loop
                ):
                    self.logger.debug(
                ...
                if GLOBAL_PLAYWRIGHT_API_OBJECT and not should_recreate:
                    try:
                        ...
                        self.logger.debug(
                            f"Detected invalid playwright instance: {type(e).__name__}. Creating new instance."
                        )
                        should_recreate = True
    ...
    async def setup_browser_via_passed_objects(self) -> None:
        ...
        if self.browser_context:
            try:
                ...
                if (
                    ...
                    and not self.browser_context.browser.is_connected()
                ):
                    self.browser_context = None
            except Exception:
        ...
        if self.browser or self.browser_context:
            self.logger.info(
                f"🎭 Connected to existing user-provided browser: {self.browser_context}"
            )
    ...
    async def setup_new_browser_context(self) -> None:
            ...
            if not self.browser_profile.user_data_dir:
                ...
                try:
                    async with asyncio.timeout(10):  # Reduced timeout from 30s to 10s
                        self.browser = (
                            self.browser
                            or await self.playwright.chromium.launch(
                ...
                try:
                    ...
                    try:
                        ...
                        user_data_dir_chrome_version = (
                            (self.browser_profile.user_data_dir / "Last Version")
                            .read_text()
                            .strip()
                        )
                    except Exception:
                    ...
                    reason = (
                        "due to bad"
                        if "Failed parsing extensions" in str(e)
                        else "for unknown reason with"
                    )
                    ...
                    browser_channel = (
                        ...
                        if self.browser_profile.executable_path
                            ...
                            self.browser_profile.channel or BROWSERUSE_DEFAULT_CHANNEL
                        ).name.lower()
                    )
                    self.logger.error(
        ...
        if new_chrome_procs and not self.browser_pid:
            ...
            self.logger.info(
                f" ↳ Spawned browser_pid={self.browser_pid} {_log_pretty_path(new_chrome_procs[0].cmdline()[0])}"
            )
            self.logger.debug(
                " ".join(new_chrome_procs[0].cmdline())
            )  # print the entire launch command for debugging
            self._set_browser_keep_alive(
                False
        ...
        self.logger.debug(
            f"🪢 Browser {self._connection_str} connected {self.browser or self.browser_context}"
        )
    ...
    async def _setup_current_page_change_listeners(self) -> None:
        ...
        try:
            ...
            self.logger.warning(
                f"⚠️ Failed to register init script for tab focus detection: {e}"
            )
    ...
    def _reset_connection_state(self) -> None:
        ...
        if self.browser_pid:
            try:
                ...
                assert proc_is_alive and "--remote-debugging-port" in " ".join(
                    proc.cmdline()
                )
            except Exception:
                self.logger.info(
                    f" ↳ Browser browser_pid={self.browser_pid} process is no longer running"
                )
    ...
    async def get_current_page(self) -> Page:
        ...
        ), f"{self} Failed to find or create a new page for the agent"
        assert (
            self.human_current_page is not None
        ), f"{self} Failed to find or create a new page for the human"
    ...
    async def remove_highlights(self):
        ...
        try:
            ...
            self.logger.debug(
                f"⚠️ Failed to remove highlights (this is usually ok): {type(e).__name__}: {e}"
            )
                ...
                if self.browser_profile.downloads_path:
                    try:
                        ...
                        download_path = os.path.join(
                            self.browser_profile.downloads_path, unique_filename
                        )
    ...
    async def get_tabs_info(self) -> list[TabInfo]:
        ...
        for page_id, page in enumerate(self.browser_context.pages):
            try:
                ...
                self.logger.debug(
                    f"⚠️ Failed to get tab info for tab #{page_id}: {_log_pretty_url(page.url)} (ignoring)"
                )
                tab_info = TabInfo(
                    page_id=page_id,
                    url="about:blank",
    ...
    async def _save_cookies_to_file(
        self, path: Path, cookies: list[dict[str, Any]] | None
    ) -> None:
        ...
        try:
            cookies_file_path = (
                Path(path or self.browser_profile.cookies_file).expanduser().resolve()
            )
            ...
            self.logger.info(
                f"🍪 Saved {len(cookies)} cookies to cookies_file= {_log_pretty_path(cookies_file_path)}"
            )
        except Exception as e:
            self.logger.warning(
    ...
    async def _save_storage_state_to_file(
        self, path: Path, storage_state: dict[str, Any] | None
    ) -> None:
        try:
            ...
            if json_path.exists():
                try:
                    ...
                    merged_storage_state = merge_dicts(
                        existing_storage_state, storage_state
                    )
                except Exception as e:
                    self.logger.error(
            ...
            self.logger.info(
                f'🍪 Saved {len(storage_state["cookies"])} cookies to storage_state= {_log_pretty_path(json_path)}'
            )
        except Exception as e:
            self.logger.warning(
    ...
    async def save_storage_state(self, path: Path | None = None) -> None:
        ...
        if not (
            path
            or self.browser_profile.storage_state
            or self.browser_profile.cookies_file
        ):
            return
        ...
        if path:
            if path.name == "storage_state.json":
                ...
                await self._save_storage_state_to_file(
                    path.parent / "storage_state.json", storage_state
                )
        ...
        if isinstance(self.browser_profile.storage_state, (str, Path)):
            await self._save_storage_state_to_file(
                self.browser_profile.storage_state, storage_state
            )
            return
    ...
    async def load_storage_state(self) -> None:
        ...
        if self.browser_profile.cookies_file:
            ...
            if not cookies_path.is_absolute():
                cookies_path = (
                    Path(self.browser_profile.downloads_path or ".")
                    .expanduser()
                    .resolve()
                    / cookies_path.name
                )
        ...
        if self.browser_profile.storage_state:
            ...
            try:
                assert isinstance(
                    storage_state, dict
                ), f"Got unexpected type for storage_state: {type(storage_state)}"
    ...
    async def _check_and_handle_navigation(self, page: Page) -> None:
        ...
        if not self._is_url_allowed(page.url):
            self.logger.warning(
                f"⛔️ Navigation to non-allowed URL detected: {page.url}"
            )
            try:
                await self.go_back()
            except Exception as e:
                self.logger.error(
                    f"⛔️ Failed to go back after detecting non-allowed URL: {type(e).__name__}: {e}"
                )
    ...
    async def close_current_tab(self):
        ...
        try:
            ...
            self.logger.debug(
                f"⛔️ Error during close_current_tab: {type(e).__name__}: {e}"
            )
    ...
    async def _get_updated_state(self, focus_element: int = -1) -> BrowserStateSummary:
        ...
        try:
            ...
            self.logger.error(
                f"❌ Failed to update browser_state_summary: {type(e).__name__}: {e}"
            )
    ...
    async def switch_to_tab(self, page_id: int) -> Page:
        ...
        try:
            ...
            self.logger.warning(
                f"⚠️ New page failed to fully load: {type(e).__name__}: {e}"
            )
    ...
    async def create_new_tab(self, url: str | None = None) -> Page:
        ...
        if url:
            try:
                ...
                self.logger.error(
                    f"❌ Error navigating to {url}: {type(e).__name__}: {e}"
                )
