...
def log_response(response: AgentOutput, registry=None, logger=None) -> None:
    ...
    logger.info(f"Thinking: {response.current_state.thinking}")
...
class Agent(Generic[Context]):
    ...
    def __init__(
        ...
        if browser_session:
            ...
            if (
                browser_session.browser_profile.keep_alive
                and not browser_session.initialized
            ):
                self.logger.error(
    ...
    def logger(self) -> logging.Logger:
        ...
        _browser_session_id = (
            self.browser_session.id if self.browser_session else self.id
        )
        _current_page_id = str(
            id(self.browser_session and self.browser_session.agent_current_page)
        )[-2:]
        ...
        try:
            ...
            def is_valid_raw_response(response, expected_answer: str) -> bool:
                ...
                try:
                    ...
                    if expected_answer.lower() not in answer:
                        self.logger.debug(
                            f"🛠️ Tool calling method {method} failed: expected '{expected_answer}', got '{answer}'"
                        )
                        return False
                    ...
                    self.logger.debug(
                        f"🛠️ Tool calling method {method} failed: Failed to parse JSON content: {e}"
                    )
                    return False
            ...
            if method == "raw" or method == "json_mode":
                ...
                structured_llm = self.llm.with_structured_output(
                    CapitalResponse, include_raw=True, method=method
                )
                response = structured_llm.invoke(
                    [HumanMessage(content=CAPITAL_QUESTION)]
                )

                if not response:
                    self.logger.debug(
                        f"🛠️ Tool calling method {method} failed: empty response"
                    )
                    return False
                ...
                if not isinstance(parsed, CapitalResponse):
                    self.logger.debug(
                        f"🛠️ Tool calling method {method} failed: LLM responded with invalid JSON"
                    )
                    return False

                if EXPECTED_ANSWER not in parsed.answer.lower():
                    self.logger.debug(
                        f"🛠️ Tool calling method {method} failed: LLM failed to answer test question correctly"
                    )
                    return False
                return True
            ...
            self.logger.debug(
                f"🛠️ Tool calling method '{method}' test failed: {type(e).__name__}: {str(e)}"
            )
            return False
    ...
    async def _test_tool_calling_method_async(self, method: str) -> tuple[str, bool]:
        ...
        result = await loop.run_in_executor(
            None, self._test_tool_calling_method, method
        )
    ...
    def _detect_best_tool_calling_method(self) -> str | None:
        ...
        try:
            ...
            async def test_all_methods():
                tasks = [
                    self._test_tool_calling_method_async(method)
            ...
            for i, method in enumerate(methods_to_try):
                if isinstance(results[i], tuple) and results[i][1]:  # (method, success)
                    ...
                    self.llm._verified_tool_calling_method = (
                        method  # Cache on LLM instance
                    )
                    elapsed = time.time() - start_time
                    self.logger.debug(
                        f"🛠️ Tested LLM in parallel and chose tool calling method: [{method}] in {elapsed:.2f}s"
                    )
                    return method

        except Exception as e:
            self.logger.debug(
                f"Parallel testing failed: {e}, falling back to sequential"
            )
            ...
            for method in methods_to_try:
                if self._test_tool_calling_method(method):
                    ...
                    elapsed = time.time() - start_time
                    self.logger.debug(
                        f"🛠️ Tested LLM and chose tool calling method: [{method}] in {elapsed:.2f}s"
                    )
                    return method

        # If we get here, no methods worked
        raise ConnectionError(
            "Failed to connect to LLM. Please check your API key and network connection."
        )
    ...
    def _set_tool_calling_method(self) -> ToolCallingMethod | None:
        ...
        if self.settings.tool_calling_method != "auto":
            ...
            if (
                ...
                self.llm._verified_api_keys = True
                self.llm._verified_tool_calling_method = (
                    self.settings.tool_calling_method
                )
        ...
        if known_method is not None:
            ...
            if (
                ...
                self.llm._verified_api_keys = True
                self.llm._verified_tool_calling_method = (
                    known_method  # Cache on LLM instance
                )
                self.logger.debug(
        ...
        try:
            ...
            if browser_state_summary:
                ...
                self._make_history_item(
                    model_output, browser_state_summary, result, metadata
                )
    ...
    def _log_step_context(self, current_page, browser_state_summary) -> None:
        ...
        url_short = (
            current_page.url[:50] + "..."
            if len(current_page.url) > 50
            else current_page.url
        )
        interactive_count = (
            len(browser_state_summary.selector_map) if browser_state_summary else 0
        )
        self.logger.info(
    ...
    def _log_llm_call_info(
        self, input_messages: list[BaseMessage], method: str
    ) -> None:
        ...
        tool_count = (
            len(self.ActionModel.model_fields) if hasattr(self, "ActionModel") else 0
        )
    ...
    async def multi_act(
        ...
        for i, action in enumerate(actions):
            ...
            try:
                ...
                self.logger.info(
                    f"☑️ Executed action {i + 1}/{len(actions)}: {action_name}({action_params})"
                )
