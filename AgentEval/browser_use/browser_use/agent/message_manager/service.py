...
def _log_format_agent_output_content(tool_call: dict) -> str:
    ...
    try:
        ...
        if "action" in args and args["action"]:
            first_action = (
                args["action"][0]
                if isinstance(args["action"], list) and args["action"]
                else args["action"]
            )
            if isinstance(first_action, dict):
                action_name = (
                    next(iter(first_action.keys())) if first_action else "unknown"
                )
...
def _log_extract_message_content(
    message: BaseMessage, is_last_message: bool, metadata: MessageMetadata | None = None
) -> str:
        ...
        try:
            ...
            for i, m in enumerate(self.state.history.messages):
                try:
                    ...
                    content = _log_extract_message_content(
                        m.message, is_last_message, m.metadata
                    )

                    # Format the message line(s)
                    lines = _log_format_message_line(
                        m, content, is_last_message, terminal_width
                    )
